...
function generarPaginaUnidad(unidad,fileName,numeral){
	...
	var	content = parseUnidad(unidad);
	...
	if(elementos.length > 0){
		...
		for (var i=0; i < elementos.length; i++) {
			if(elementos[i].getElementsByTagName(what_to_get)[0]){
				if(parent_n !== '')var elementoNumeral = parent_n+'.'+(i+1);
				if(parent_n === '')var elementoNumeral = '';
				var elementoTitulo = elementos[i].getElementsByTagName(what_to_get)[0].childNodes[0].nodeValue;

				var elementoUrl = elementoTitulo.latinize().replace(/ /g,'-').toLowerCase();
...
function parseUnidad(elementGroup){
	var output = '<div class="unidad">';
	...
	if(elementGroup.childNodes){
		...
		output = titulo+objetivos+intro+apartados; 
	}
	return output+'</div>';
}
...
function getParts(element, what_to_get){
	...
	if(element.getElementsByTagName(what_to_get)){
		...
		for (var i=0; i < parts.length; i++) {
			if(parts[i].getElementsByTagName(what_to_get+'_titulo')[0])var partTitulo = '<h'+titleLvl+'>'+parts[i].getElementsByTagName(what_to_get+'_titulo')[0].childNodes[0].nodeValue+'</h'+titleLvl+'>';
